...
function App() {
  ...
  const [infos, setInfos] = useState([100, 20, 40, 50])
      ...
      <Row gutter={24} style={{marginBottom: 24}}>
        <Col span={24}>
          <Card title="流量信息栏">
            {infos.length > 0 ? 
              <>
                <div>隧道内车辆总数：<b>{infos[0]}</b></div>
                <div>隧道内第一段车辆数：<b>{infos[1]}</b></div>
                <div>隧道内第二段车辆数：<b>{infos[2]}</b></div>
                <div>每小时车流量：<b>{infos[3]}</b></div>
              </>:
              <span>暂无数据</span>
            }
